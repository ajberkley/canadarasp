...
if [ -z $VOL_ID ]; then
  ...
  export VOL_ID=`aws ec2 create-volume --no-encrypted --availability-zone $MY_ZONE --size 30 --volume-type standard --tag-specifications 'ResourceType=volume,Tags=[{Key=Name,Value='$DBOXNAME'}]' | head -1 | awk '{ print $7 }'`
  if [ -z $VOL_ID ]; then
