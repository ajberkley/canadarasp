...
if [ -f hrdpswindgram0.png ]; then
   aws cloudwatch put-metric-data --namespace canadarasp --metric-name hrdps_monitor --value 1
fi
rm hrdpswindgram0.png
...
if [ -f gdpswindgram0.png ]; then
   aws cloudwatch put-metric-data --namespace canadarasp --metric-name gdps_monitor --value 1
fi
rm gdpswindgram0.png
