...
(defparameter *params-and-names*
  `(("sfcwind" "Wind at 10m" ("UGRD_TGL_10" "VGRD_TGL_10") :wind ,*wind-color-scale* "[km/hr]")
    ...
    ("hwcritagl" "Top of Life above ground level" "HCRIT_SFC_0" :mag ,*hcrit-color-scale* "[m AGL]")
...
(defparameter *domain* "continental")

(defun tile-iterator ()
  (let* ((ulx (if (string= *domain* "continental") -152 -134)) ;; rememer this shoudl be even!
	 (uly (if (string= *domain* "continental") 71 55)) ;; remember this should be odd!
	 (lrx (if (string= *domain* "continental") -42 -112))
	 (lry (if (string= *domain* "continental") 27 45)) ;; rememver this shoudl be odd!
	 (minx ulx)
	 (maxx lrx)
	 (miny lry)
	 (maxy uly)
	 ...
	 (y (- uly 2)))
    (lambda ()
      (if (and (<= minx x (- maxx 2)) (<= miny y (- maxy 2)))
	  (prog1
	      (list x y (+ x 2) (+ y 2))
	    (incf x 2)
	    (when (> x (- maxx 2))
...
(if (string= (cadr *posix-argv*) "--only-generate-header-footer")
    (map nil (lambda (hour) (do-it-if-necessary hour :only-generate-header-footer t)) (iter (for hour from 0 below 49 by 8) (collect hour)))
