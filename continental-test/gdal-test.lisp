...
(defun input-file-directory () "/mnt/input/")
...
(defparameter *surface-pressure-color-scale* (make-basic-color-scale 920f0 1080f0))
...
(defun write-default-title (parameter forecast-init-year forecast-init-month forecast-init-day forecast-init-hour filename &key (view nil))
  (let ((second-line (format nil "HRDPS initialized ~A-~2,'0d-~A (~2,'0d:00) UTC"
...
(defun tmp-filename (&optional file (extension ""))
  (if file
    (format nil "~A~A-~A-~A.~A" *unique-identifier* (incf *tmp-fileindex*) file (sxhash (list file *tmp-fileindex* extension)) extension)
    (format nil "~A~A.tmp" *unique-identifier* (incf *tmp-fileindex*))))
...
(defparameter *params-and-names*
  `(("sfcwind" "Wind at 10m" ("UGRD_TGL_10" "VGRD_TGL_10") :wind ,*wind-color-scale* "[km/hr]")
    ...
    ("vwind0" "Vertical component of wind at 230m" "VVEL_ISBL_1000" :mag ,*vvel-color-scale* "[m/s]")
    ("vwind1" "Vertical component of wind at 1600m" "VVEL_ISBL_0850" :mag ,*vvel-color-scale* "[m/s]")
    ("vwind2" "Vertical component of wind at 3100m" "VVEL_ISBL_0700" :mag ,*vvel-color-scale* "[m/s]")
    ("cloud" "Total Cloud Cover" "TCDC_SFC_0" :mag ,*cloud-color-scale* "[%]")
    ("CAPE" "Convective Available Potential Energy (CAPE)" "CAPE_ETAL_10000" :mag ,*cape-color-scale* "[J/kg]")
    ("hwcritagl" "Top of Life above ground level" "HWCRITAGL" :mag ,*hcrit-color-scale* "[m AGL]")
    ("bldepth" "Boundary layer height above ground level" "BLDEPTH" :mag ,*hcrit-color-scale* "[m AGL]")
    ...
    ("sfcdewpt" "Surface Dew Point Temperature (2m AGL)" "DEPR_TGL_2" :mag ,*surface-temperature-color-scale* "[C]" ;; ,(lambda (x) (- x 273.15))
		)
    ("sfcpres" "Surface Pressure (mbar)" "PRES_SFC_0" :mag ,*surface-pressure-color-scale* "mbar" ,(lambda (x) (/ x 100.0)))))
...
(defun make-tile-directories (yyyy mm dd)
  (let ((tile-iterator (tile-iterator)))
    (iter (for tile = (funcall tile-iterator))
	  ...
	  ;; (sb-ext:run-program "/bin/mkdir" (list "-p" directory))
	  )))
...
(defmacro with-drawable-image ((draw-func image y-size x-size &optional debug) &body body)
  (let ((debug-gensym (when debug (gensym)))
	(displaced (gensym))
	(size (gensym))
	(offset (gensym)))
    `(let* ((,image (png::make-image ,y-size ,x-size 4 8))
	    (,displaced (array-displacement ,image))
	    (,size (* 4 ,x-size))
	    ,@(when debug (list (list debug-gensym 0)))) ;; our output image
       (declare (type (array (unsigned-byte 8) (* * 4)) ,image)
		(type (simple-array (unsigned-byte 8) (*)) ,displaced))
       (labels ((,draw-func (x y red green blue) ;; 2.7 seconds
		  (declare (type (integer -1000000 100000000) x y))
		  (declare (type (unsigned-byte 8) red green blue))
		  (let ((,offset (+ (* 4 x) (* ,size y))))
...
(defun draw-magnitude (file color-scale filename &optional (scale #'identity))
  ...
  (let ((filename-warped (tmp-filename file)))
...
(defun combine-winds-warp-and-return-new-files (ugrib vgrib)
  (let ((both-grib-rotated (tmp-filename "both-rotated" "grib2"))
    ...
    (print/run-program "/usr/local/bin/gdalwarp" (list "-overwrite" "-r" "bilinear" "-t_srs" "EPSG:3857" 
    ...
    (print/run-program "/usr/local/bin/gdalwarp" (list "-overwrite" "-r" "bilinear" "-t_srs" "EPSG:3857" "-of" "GTiff" "-wo" "SAMPLE_GRID=YES"
						       "-dstnodata" (format nil "~A" +dst-no-data-double+)
...
(defun draw-winds* (ufile-input vfile-input filename &key (scale 10) (color-scale *wind-color-scale*))
  "Slightly faster (47 seconds) at scale 8, but ugly arrows.
   Scale 9 is 60 seconds, nice arrows and scale 10 is 66 seconds
   Can run with dynamic-space-size = 2GB
   Initial file generation takes 10 seconds, so total time is 80 seconds"
  (declare (optimize (speed 3))
	   (type (integer 1 11) scale))
  (cl-gdal::maybe-initialize-gdal-ogr)
  (destructuring-bind (ufile vfile)
	...
	(let ((tile-iterator (tile-iterator)))
	  ...
	  (loop for tile  = (funcall tile-iterator)
		 ...
		 ;;(format t "~A: ~A->~A, ~A->~A~%" tile x-pixel-min x-pixel-max y-pixel-min y-pixel-max)
		 (let ((new-y-size (* scale (the image-size (- y-pixel-max y-pixel-min))))
		       (new-x-size (* scale (the image-size (- x-pixel-max x-pixel-min)))))
			  ...
			  (loop for x fixnum from x-pixel-min below x-pixel-max
				 ...
				 (when (not (= mag +dst-no-data-single+))
				   (destructuring-bind (red green blue)
				     ...
				     (loop for y-new fixnum from (* scale (- y y-pixel-min)) to (- (* scale (+ 1 (- y y-pixel-min))) 1)
					do
					  (loop for x-new fixnum from (* scale (- x x-pixel-min)) to (- (* scale (+ 1 (- x x-pixel-min))) 1)
					     do
					       (draw x-new y-new red green blue))))
				   (draw-lines (manipulate-object* *prototype-arrow*
								   :rotate (- (float (atan v u) 0f0)) ;; negative because y goes down the image
...
(defparameter *forecast-init-year* (parse-integer (sb-posix:getenv "YEAR")))
(defparameter *forecast-init-month* (parse-integer (sb-posix:getenv "MONTH")))
(defparameter *forecast-init-day* (parse-integer (sb-posix:getenv "DAY")))
(defparameter *forecast-init-hour* (parse-integer (sb-posix:getenv "HOUR")))
...
(defun do-it-if-necessary (forecast-hour &key (forecast-init-year *forecast-init-year*) (forecast-init-month *forecast-init-month*) (forecast-init-day *forecast-init-day*) (forecast-init-hour *forecast-init-hour*)
					   (generate-header-footer t) (params-and-names *params-and-names*))
  ...
  (let* ((t0 (local-time:encode-timestamp 0 0 0 forecast-init-hour forecast-init-day forecast-init-month forecast-init-year :timezone local-time:+gmt-zone+))
    ...
    (make-tile-directories forecast-init-year forecast-init-month forecast-init-day)
    (labels ((gen-input-filename/s (filelabel/s)
	       (labels ((d (filelabel)
			  (format nil "~ACMC_hrdps_~A_~A_ps2.5km_~A~2,'0d~2,'0d~2,'0d_P~3,'0d-00.grib2"
				  (input-file-directory) *domain* filelabel forecast-init-year forecast-init-month forecast-init-day forecast-init-hour forecast-hour)))
		 (if (listp filelabel/s) (mapcar #'d filelabel/s) (d filelabel/s))))
	     (output-file-name (param &optional (tag "body"))
	       (format nil "~A/~A_~A-~2,'0d-~2,'0d_~2,'0d00.~A.png" (directory-from-date forecast-init-year forecast-init-month forecast-init-day)
		       param yearpt monthpt daypt hourpt tag))
	     (header/footer-file-name (param tag)
	       (format nil "~A/~A_~2,'0d-~2,'0d-~2,'0d.~A.png" (base-directory) param forecast-init-year forecast-init-month forecast-init-day tag))
		 ...
		 (ecase type
		   (:wind
		    (draw-winds* (car input-filename/s) (cadr input-filename/s) output-filename :color-scale color-scale))
		   (:mag
		    (draw-magnitude input-filename/s color-scale output-filename scale)))
		 (when generate-header-footer
		   (draw-color-scale color-scale (print (header/footer-file-name param "foot"))  :units units :process #'identity)
	...
	(let ((*unique-identifier* forecast-hour))
	  (map nil (lambda (x)
		     (destructuring-bind (param full-name filelabel type color-scale units &optional (scale #'identity)) x
		       (handle param full-name filelabel type color-scale units scale))) params-and-names))))))
